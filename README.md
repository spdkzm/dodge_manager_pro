...
### v1.0.x (Fix: 集計機能の正常化)
**重要: データベース保存と集計ロジックの修正**

* **データ保存の不具合修正:**
  * 試合ログを保存する際、内部データのキー名不一致により、詳細情報（選手名、アクション内容など）が欠落して保存される問題を修正しました。
* **集計ロジックの改善:**
  * 分析画面において、特定の試合を選択した際に集計が「0」になる問題を修正しました。
  * データ取得フローを見直し、期間集計と試合別集計で統一されたデータ形式（DB生データ）を使用するように変更しました。

> **⚠️ 注意:**
> 本修正適用前に保存された「集計が0になっている試合記録」は、データの中身が空の状態です。アプリ内の「設定 > 試合結果の削除」から該当する試合を削除し、再入力をお願いします。
