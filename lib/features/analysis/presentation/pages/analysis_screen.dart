...
class _AnalysisScreenState extends ConsumerState<AnalysisScreen> with TickerProviderStateMixin {
  ...
  void _showEditLogDialog({LogEntry? log}) {
    ...
    showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, setStateDialog) {
      ...
      final selectedDef = definitions.firstWhere(
            ...
            return InkWell( // ★修正: タップ可能に
              ...
              child: Container(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Text(log.action, style: const TextStyle(color: Colors.black54, fontSize: 11, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis))
                  ])
              ),
            );
          }
          ...
          return InkWell(
              ...
              child: Container(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Text("${log.action} $resultText", style: const TextStyle(fontSize: 13), overflow: TextOverflow.ellipsis)),
  ...
  // ★復元: 選手詳細ダイアログ表示
  Widget _showPlayerDetail(PlayerStats player) {
    // 定義を取得するためコントローラーから読み込む
    ...
    return const SizedBox(); // 戻り値としては使わないがWidgetを返す必要があるため
  }
